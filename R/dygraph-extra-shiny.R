...
dyDownloadGroup =
  ...
  function(groupid, label, downIDs, downAll = TRUE, usetitle = TRUE, asbutton = FALSE) {
    if (is.null(names(downIDs))) {
      names(downIDs) = downIDs
    }
    downlist = list()
    for (i in 1:length(downIDs)) {
      downlist[[i]] = tags$li(
        dyDownload(downIDs[i], names(downIDs)[i], usetitle = usetitle, asbutton = asbutton)
      )
    }
    ...
    tags$div(
      id = groupid,
      class = "form-group dydownload-group",
      tags$label(label),
